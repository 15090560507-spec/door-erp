"""
西州将军铜门 - 生产图纸协同系统 纯业务逻辑层
- 已去除所有 Streamlit UI 代码
- FastAPI 后端位于 backend/ 目录
- 本文件保留核心逻辑作为参考
"""
import sys
import os
...
class UserDatabaseManager:
    def __init__(self, file_path=USERS_DB_FILE):
...
class TaskDatabaseManager:
    def __init__(self, file_path=TASKS_DB_FILE):
        self.file_path = file_path
        if not os.path.exists(self.file_path):
            self.save([])
...
class CustomOptionsManager:
    def __init__(self, file_path=CUSTOM_OPTIONS_FILE):
        self.file_path = file_path
...
def draw_door_in_frame(drawer: EzdxfDrawer, view_name: str, p: Dict, is_back: bool,
    ...
    if door_type == "单门":
        ...
        if not is_back:
            drawer.draw_dim(off((mother_panel_x1, panel_y_bot - 100)),
                             off((mother_panel_x2, panel_y_bot - 100)),
                             off((mother_panel_x1 - 100, panel_y_bot - 150)),
                             0, 'YQ_DIM', f"母门宽 {mother_width}")
        ...
        drawer.draw_poly([off((lx1, panel_y_bot)), off((lx2, panel_y_bot)),
                           off((lx2, panel_y_top)), off((lx1, panel_y_top))], 'A-DOOR-PANEL')
        drawer.draw_poly([off((lmx1, panel_y_bot)), off((lmx2, panel_y_bot)),
                           off((lmx2, panel_y_top)), off((lmx1, panel_y_top))], 'A-DOOR-PANEL')
        drawer.draw_poly([off((rmx1, panel_y_bot)), off((rmx2, panel_y_bot)),
                           off((rmx2, panel_y_top)), off((rmx1, panel_y_top))], 'A-DOOR-PANEL')
        drawer.draw_poly([off((rx1, panel_y_bot)), off((rx2, panel_y_bot)),
                           off((rx2, panel_y_top)), off((rx1, panel_y_top))], 'A-DOOR-PANEL')
        panel_positions.extend([(lx1, lx2), (lmx1, lmx2), (rmx1, rmx2), (rx1, rx2)])

        if not is_back:
            drawer.draw_dim(off((lmx1, panel_y_bot - 150)), off((rmx2, panel_y_bot - 150)),
                             off((lmx1 + mid_total_width / 2, panel_y_bot - 200)),
                             0, 'YQ_DIM', f"中门宽度 {mid_total_width}mm")
        ...
        drawer.draw_poly([off((lx1, panel_y_bot)), off((lx2, panel_y_bot)),
                           off((lx2, panel_y_top)), off((lx1, panel_y_top))], 'A-DOOR-PANEL')
        if has_pillar:
            drawer.draw_poly([off((lpx1, panel_y_bot)), off((lpx2, panel_y_bot)),
                               off((lpx2, panel_y_top)), off((lpx1, panel_y_top))], 'A-DOOR-FRAME')
        drawer.draw_poly([off((lmx1, panel_y_bot)), off((lmx2, panel_y_bot)),
                           off((lmx2, panel_y_top)), off((lmx1, panel_y_top))], 'A-DOOR-PANEL')
        drawer.draw_poly([off((rmx1, panel_y_bot)), off((rmx2, panel_y_bot)),
                           off((rmx2, panel_y_top)), off((rmx1, panel_y_top))], 'A-DOOR-PANEL')
        if has_pillar:
            drawer.draw_poly([off((rpx1, panel_y_bot)), off((rpx2, panel_y_bot)),
                               off((rpx2, panel_y_top)), off((rpx1, panel_y_top))], 'A-DOOR-FRAME')
        drawer.draw_poly([off((rx1, panel_y_bot)), off((rx2, panel_y_bot)),
                           off((rx2, panel_y_top)), off((rx1, panel_y_top))], 'A-DOOR-PANEL')
        panel_positions.extend([(lx1, lx2), (lmx1, lmx2), (rmx1, rmx2), (rx1, rx2)])
    ...
    if use_light_size and light_w > 0 and ((nk_choice == "内开" and not is_back) or (nk_choice == "外开" and is_back)):
        dims_h.append(("见光宽", left_width + left_gap, dw - right_width - right_gap,
                        -200, True, f"见光宽 {light_w}"))
    ...
    for name, x1, x2, y_offset, condition, text in dims_h:
        if condition:
            drawer.draw_dim(off((x1, y_offset)), off((x2, y_offset)),
                             off((x1 + (x2 - x1) / 2, y_offset - 50)), 0, 'YQ_DIM', text)

    for name, y1, y2, x_offset, condition, text in dims_v:
        if condition:
            drawer.draw_dim(off((outer_right + x_offset, y1)), off((outer_right + x_offset, y2)),
                             off((outer_right + x_offset + 50, y1 + (y2 - y1) / 2)),
                             rad90, 'YQ_DIM', text)
    ...
    if is_hinge_visible:
        if door_type == "单门":
            ...
            hinge_x_list.extend([left_width + 5, panel_positions[0][1] + 5,
                                 panel_positions[2][1] + 5, dw - right_width - 5])
    ...
    if current_handle == "标配拉手":
        ...
        if door_type == "单门":
            ...
            handles_to_draw.extend([(panel_positions[0][1] - 60, "ZBPLS"),
                                     (panel_positions[1][0] + 60, "YBPLS")])

        elif door_type in ["折叠四开门", "两定两开"] and len(panel_positions) >= 4:
            handles_to_draw.extend([(panel_positions[1][1] - 60, "ZBPLS"),
                                     (panel_positions[2][0] + 60, "YBPLS")])
...
# ===================== 集成系统入口 =====================
def run_integrated_system(info: Dict, checks: Dict, draw_p: Dict,
                          progress_callback=None) -> Tuple[str, Optional[io.StringIO]]:
    """生成 CAD 图纸的核心入口函数"""
    ...
    try:
        ...
        base_attrs = {
            ...
            "ZMKS": info.get("ZMKS", "按图"), "FMKS": info.get("FMKS", "按图"),
        }
        ...
        drawer.batch_add_layers({
            "A-DOOR-FRAME": 4, "A-DOOR-PANEL": 2,
            "A-DOOR-TRIM": 1, "YQ_DIM": 3, "A-DOOR-mark": 7
        })
