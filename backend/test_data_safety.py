...
check("users: 销售小A使用新默认密码", user_db.authenticate("A", "123654") is not None)
check("users: 销售小A旧默认密码失效", user_db.authenticate("A", "123") is None)
...
# 5f. A 的旧默认密码只迁移一次，人工设置的新密码不会被覆盖
users = user_db.load_all_users()
users["A"]["password"] = hash_password("123")
user_db.save(users)
user_db2 = UserDatabaseManager(
    file_path=os.path.join(DATA_DIR, "test_users.json"),
    backup_dir=os.path.join(DATA_DIR, "backups", "users"),
