...
export default function PurchasingCenter({ notify }: { notify: (message: string, error?: boolean) => void }) {
  ...
  const receive = async (summary: PurchaseOrder) => {
    ...
    try {
      ...
      const items = (order.items || []).map((item) => ({ purchase_order_item_id: item.id, quantity: Math.max(0, Number(item.remaining_receivable_quantity || 0)) })).filter((item) => item.quantity > 0);
  ...
  return <div className="space-y-4">
    <section className="border border-[#D1D1D6] bg-white">
      ...
      <div className="grid gap-2 p-3 lg:hidden">{shortages.map((row) => <article key={row.requirement_item_id} className={`border p-3 ${selected.includes(row.requirement_item_id) ? "border-[#007AFF] bg-[#F2F7FF]" : "border-[#E5E5EA] bg-[#FAFAFB]"}`}><div className="flex items-start gap-3"><input type="checkbox" className="mt-1" aria-label={`选择 ${row.production_no} ${row.material_name}`} checked={selected.includes(row.requirement_item_id)} onChange={(event) => setSelected(event.target.checked ? [...selected, row.requirement_item_id] : selected.filter((id) => id !== row.requirement_item_id))}/><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><strong className="block truncate text-sm">{row.material_name}</strong><span className="mt-1 block truncate text-xs text-[#636366]">{row.material_code} · {row.specification || "无规格"}</span></div><span className="shrink-0 text-sm font-semibold text-[#C62828]">缺 {qty(row.demand_quantity)} {row.unit}</span></div><div className="mt-3 grid grid-cols-2 gap-2 text-xs sm:grid-cols-3"><SmallInfo label="生产编号" value={row.production_no}/><SmallInfo label="要求交期" value={row.due_date || "未设置"}/><SmallInfo label="供应商建议" value={row.default_supplier || "未设置"}/></div><label className="mt-3 block text-xs text-[#636366]">采购单价<input type="number" min="0" value={unitPrices[row.material_id] || ""} onChange={(event) => setUnitPrices({...unitPrices, [row.material_id]: Number(event.target.value) || 0})} className="mt-1 h-9 w-full border border-[#C7C7CC] bg-white px-3 text-sm text-[#1C1C1E]"/></label></div></div></article>)}{!shortages.length&&<div className="p-8 text-center text-sm text-[#8E8E93]">当前没有未覆盖缺口</div>}</div>
      <div className="hidden overflow-x-auto lg:block"><table className="w-full min-w-[1050px] table-fixed text-sm"><thead className="bg-[#F2F2F7] text-left text-xs text-[#636366]"><tr>{["选择","生产编号","要求交期","物料","规格","缺口","供应商建议","采购单价"].map((name)=><th key={name} className="px-3 py-2 font-medium">{name}</th>)}</tr></thead><tbody>{shortages.map((row)=><tr key={row.requirement_item_id} className="border-t border-[#E5E5EA]"><td className="px-3 py-3"><input type="checkbox" checked={selected.includes(row.requirement_item_id)} onChange={(e)=>setSelected(e.target.checked?[...selected,row.requirement_item_id]:selected.filter((id)=>id!==row.requirement_item_id))}/></td><td className="px-3 py-3 font-medium text-[#007AFF]">{row.production_no}</td><td className="px-3 py-3">{row.due_date||"-"}</td><td className="px-3 py-3"><div>{row.material_name}</div><div className="text-xs text-[#636366]">{row.material_code}</div></td><td className="px-3 py-3">{row.specification||"-"}</td><td className="px-3 py-3 font-semibold text-[#C62828]">{qty(row.demand_quantity)} {row.unit}</td><td className="px-3 py-3">{row.default_supplier||"-"}</td><td className="px-3 py-3"><input type="number" min="0" value={unitPrices[row.material_id]||""} onChange={(e)=>setUnitPrices({...unitPrices,[row.material_id]:Number(e.target.value)||0})} className="h-8 w-24 border border-[#C7C7CC] px-2"/></td></tr>)}{!shortages.length&&<tr><td colSpan={8} className="p-8 text-center text-[#8E8E93]">当前没有未覆盖缺口</td></tr>}</tbody></table></div>
    ...
    <section className="border border-[#D1D1D6] bg-white"><header className="border-b border-[#E5E5EA] p-4"><h2 className="font-semibold">采购单与到货进度</h2></header><div className="divide-y divide-[#E5E5EA]">{orders.map((order)=><div key={order.id} className="p-4"><div className="flex flex-wrap items-center gap-3"><strong>{order.order_no}</strong><span className="text-sm">{order.supplier}</span><Status text={order.status}/><span className="text-xs text-[#636366]">预计 {order.expected_date||"-"} · 已登记 {qty(order.registered_quantity||0)}/{qty(order.ordered_quantity||0)} · 待检 {qty(order.pending_inspection_quantity||0)} · 已入库 {qty(order.received_quantity||0)} · ¥{Number(order.total_amount||0).toFixed(2)}</span><div className="flex-1"/>{order.status==="草稿"&&<><button disabled={busy} onClick={()=>void run(()=>confirmPurchaseOrder(order.id))} className="ui-button ui-button--primary">确认下单</button><button disabled={busy} onClick={()=>setPendingDelete(order)} className="ui-button ui-button--danger"><Trash2 size={14}/>删除草稿</button></>}{["已下单","部分到货"].includes(order.status)&&<>{Number(order.remaining_receivable_quantity||0)>0?<button disabled={busy} onClick={()=>void receive(order)} className="ui-button ui-button--primary">登记全部剩余到货</button>:Number(order.pending_inspection_quantity||0)>0?<span className="text-xs font-medium text-[#B26A00]">已全部登记，等待检验</span>:null}<button disabled={busy} onClick={()=>void run(()=>cancelPurchaseOrder(order.id))} className="ui-button ui-button--danger">取消剩余</button></>}</div></div>)}{!orders.length&&<div className="p-8 text-center text-sm text-[#8E8E93]">暂无采购单</div>}</div></section>
...
function SmallInfo({label,value}:{label:string;value:string}){return <div className="min-w-0"><span className="block text-[#8E8E93]">{label}</span><span className="mt-0.5 block truncate text-[#3A3A3C]">{value}</span></div>}
