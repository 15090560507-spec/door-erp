import { Banknote, CalendarDays, MapPin, Package, Pencil, Phone, ReceiptText, RotateCcw, UserRound, XCircle } from "lucide-react";
...
import OrderAttachmentSummary from "./OrderAttachmentSummary";
  ...
  return (
    <article className="order-summary">
      ...
      <section className="order-summary__facts" aria-label="订单基本信息">
        ...
        <div><Phone size={17} /><span>联系电话</span><strong>{order.customer_phone || "-"}</strong><small>客户联系方式</small></div>
        <div><Package size={17} /><span>商品类别</span><strong>{order.product_category || "-"}</strong><small>订单产品分类</small></div>
      ...
      <section className="order-summary__section">
        ...
        <div className="order-summary__lines">{order.lines.map((line) => <div className={`order-summary-line${line.source_changed ? " has-warning" : ""}`} key={line.id || line.task_id}><div className="order-summary-line__main"><span className="order-summary-line__title"><strong>{line.line_code || `${order.order_no}-${String(line.line_no).padStart(2, "0")}`} · {line.product_name || line.door_type}</strong><StatusChip tone={line.source_type === "manual" ? "neutral" : "green"}>{line.source_type === "manual" ? "手工录入" : "终审图纸"}</StatusChip>{!line.quote_id && line.source_type === "drawing" && <StatusChip tone="amber">缺报价</StatusChip>}</span><OrderLineDetails line={line} /><small>{line.source_type === "drawing" ? `图纸 ${line.task_id} · 技术版本 ${line.drawing_revision || "未记录"}` : line.remark || "临时业务明细"}</small>{line.source_changed && <span className="order-summary-line__warning">来源图纸已变化，请取消原订单并重新确认。</span>}</div><div className="order-summary-line__amount"><span>{line.quantity} {line.unit}</span><strong>¥{line.amount.toLocaleString()}</strong></div></div>)}</div>
      </section>
      <OrderAttachmentSummary orderId={order.id} attachments={order.attachments || []} />
