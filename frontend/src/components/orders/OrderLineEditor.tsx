...
export default function OrderLineEditor({ line, index, disabled, onChange, onRemove }: { line: SalesOrderEditorLine; index: number; disabled: boolean; onChange: (changes: Partial<SalesOrderEditorLine>) => void; onRemove: () => void }) {
  ...
  const warnings = [line.source_type === "drawing" && !line.quote_id ? "请选择对应报价" : "", line.source_changed ? "图纸版本已变化，保存草稿后再确认" : "", line.width <= 0 || line.height <= 0 ? "宽、高必须大于 0" : "", line.quantity <= 0 ? "数量必须大于 0" : ""].filter(Boolean);
  return (
    <article className={`order-line-editor${warnings.length ? " has-warning" : ""}`}>
      ...
      <div className="order-line-editor__fields order-line-editor__fields--technical">
        {([ ["trim_type", "门套类型"], ["main_door_style", "主门款式"], ["lock_type", "锁具"], ["handle", "拉手"], ["hinge", "铰链"], ["material", "材质"], ["item_remark", "商品行备注"] ] as const).map(([key, label]) => <Field key={key} label={label}><input disabled={disabled} value={line.technical_details[key] || ""} onChange={(event) => onChange({ technical_details: { ...line.technical_details, [key]: event.target.value } })} /></Field>)}
      </div>
