  ...
  return (
    <article className={`order-line-editor${warnings.length ? " has-warning" : ""}`}>
      ...
      <div className="order-line-editor__fields order-line-editor__fields--technical">
        {ORDER_LINE_TECHNICAL_FIELDS.map(({ key, label }) => <Field key={key} label={label}><input disabled={disabled} value={line.technical_details[key] || ""} onChange={(event) => onChange({ technical_details: { ...line.technical_details, [key]: event.target.value } })} /></Field>)}
      </div>
