...
type EditorLine = {
  task_id: string; product_name: string; door_type: string; width: number; height: number;
  ...
  unit_price: number; remark: string; source_changed?: boolean;
};
...
export default function OrdersPage() {
  ...
  const addPicked = () => {
    ...
    const additions = selectedCandidates.filter((candidate) => !existingIds.has(candidate.task_id)).map((candidate): EditorLine => {
      ...
      return {
        ...
        quoteChoices: candidate.quotes, quantity: 1, unit_price: quote?.amount || 0, remark: "",
      };
    });
  ...
  const payload = (): SalesOrderPayload => ({
    ...
    lines: editor.lines.map((line) => ({
      ...
      unit_price: line.quote_id ? line.unit_price : null,
      remark: line.remark,
    })),
  });
      ...
      <section className="grid min-h-[680px] gap-4 xl:grid-cols-[360px_minmax(0,1fr)]">
        ...
        <div className="min-w-0 border border-[#D1D1D6] bg-white">
          {!editing ? <Empty text="选择订单查看详情，或新建订单" tall /> : <div>
            <div className="flex flex-wrap items-center gap-3 border-b border-[#E5E5EA] px-5 py-4"><div className="flex-1"><h2 className="font-semibold">{selected?.order_no || "新订单草稿"}</h2><p className="mt-1 text-xs text-[#636366]">{selected ? `版本 ${selected.version} · ${statusLabel[selected.status] || selected.status}` : "选择图纸后保存"}</p></div>{selected && <Status value={selected.status} />}{selected && ["draft", "confirmed"].includes(selected.status) && <button disabled={busy} onClick={() => setCancelOpen(true)} className="h-9 border border-[#D70015] px-3 text-sm text-[#D70015] disabled:opacity-50">取消订单</button>}<button disabled={busy || Boolean(selected && selected.status !== "draft")} onClick={openCandidates} className="h-9 border border-[#007AFF] px-3 text-sm text-[#007AFF] disabled:border-[#D1D1D6] disabled:text-[#8E8E93]">选择图纸</button><button disabled={busy || Boolean(selected && selected.status !== "draft")} onClick={() => void save()} className="h-9 border border-[#C7C7CC] px-4 text-sm disabled:text-[#8E8E93]">保存草稿</button><button disabled={busy || !editor.lines.length || Boolean(selected && selected.status !== "draft")} onClick={() => setConfirmOpen(true)} className="h-9 bg-[#007AFF] px-4 text-sm font-medium text-white disabled:bg-[#C7C7CC]">正式确认</button></div>
            <section className="border-b border-[#E5E5EA] p-5"><h3 className="text-sm font-semibold">基本信息</h3><div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4"><Field label="订单日期"><input type="date" value={editor.order_date} onChange={(event) => setEditor({ ...editor, order_date: event.target.value })} /></Field><Field label="客户"><input value={editor.customer_name} readOnly placeholder="由所选图纸带入" /></Field><Field label="项目"><input value={editor.project_name} onChange={(event) => setEditor({ ...editor, project_name: event.target.value })} /></Field><Field label="交期" required><input type="date" value={editor.delivery_date} onChange={(event) => setEditor({ ...editor, delivery_date: event.target.value })} /></Field><Field label="销售员"><input value={editor.salesperson} onChange={(event) => setEditor({ ...editor, salesperson: event.target.value })} /></Field><Field label="收货地址"><input value={editor.delivery_address} onChange={(event) => setEditor({ ...editor, delivery_address: event.target.value })} /></Field><Field label="收款模板"><input value={editor.payment_template} onChange={(event) => setEditor({ ...editor, payment_template: event.target.value })} /></Field><Field label="订单备注"><input value={editor.remark} onChange={(event) => setEditor({ ...editor, remark: event.target.value })} /></Field></div></section>
            <section className="p-5"><div className="flex items-center justify-between"><div><h3 className="text-sm font-semibold">门樘明细</h3><p className="mt-1 text-xs text-[#636366]">每张图纸对应一条明细；正式确认前每条都必须关联报价。</p></div><span className="text-sm text-[#636366]">{editor.lines.length} 项</span></div><div className="mt-4 space-y-3">{editor.lines.length ? editor.lines.map((line, index) => <div key={line.task_id} className={`border ${line.source_changed ? "border-[#D70015]" : "border-[#D1D1D6]"}`}><div className="grid gap-3 bg-[#F7F7F9] px-4 py-3 md:grid-cols-[minmax(0,1fr)_auto]"><div><div className="flex flex-wrap items-center gap-2"><strong className="text-sm">{index + 1}. {line.product_name || line.door_type}</strong><Tag text={line.drawing_status} good={line.drawing_status === "已通过"} /><Tag text={line.quote_id ? "已关联报价" : "未报价"} good={Boolean(line.quote_id)} />{line.source_changed && <Tag text="图纸已变更，需重存草稿" good={false} />}</div><p className="mt-1 text-xs text-[#636366]">{line.door_type} · {line.width} × {line.height} mm · {line.opening_direction || "未填开向"} · {line.color || "未填颜色"}</p></div>{selected?.status === "draft" || !selected ? <button onClick={() => setEditor((current) => ({ ...current, lines: current.lines.filter((_, lineIndex) => lineIndex !== index) }))} className="text-xs text-[#D70015]">移除</button> : null}</div><div className="grid gap-3 p-4 sm:grid-cols-2 xl:grid-cols-5"><Field label="关联报价"><select value={line.quote_id ? `${line.quote_id}:${line.quote_group_index || 0}` : ""} onChange={(event) => { const choice = line.quoteChoices.find((item) => `${item.quote_id}:${item.group_index}` === event.target.value); updateLine(index, { quote_id: choice?.quote_id || null, quote_group_index: choice?.group_index ?? null, unit_price: choice?.amount || 0 }); }}><option value="">未关联报价</option>{line.quoteChoices.map((choice) => <option key={`${choice.quote_id}-${choice.group_index}`} value={`${choice.quote_id}:${choice.group_index}`}>#{choice.quote_id} · {choice.quote_date} · ¥{choice.amount}</option>)}</select></Field><Field label="数量"><input type="number" min="1" value={line.quantity} onChange={(event) => updateLine(index, { quantity: Math.max(1, Number(event.target.value) || 1) })} /></Field><Field label="单价"><input type="number" min="0" value={line.unit_price} onChange={(event) => updateLine(index, { unit_price: Math.max(0, Number(event.target.value) || 0) })} /></Field><Field label="金额"><input value={`¥${(line.quantity * line.unit_price).toLocaleString()}`} readOnly /></Field><Field label="备注"><input value={line.remark} onChange={(event) => updateLine(index, { remark: event.target.value })} /></Field></div></div>) : <div className="border border-dashed border-[#C7C7CC] py-12 text-center text-sm text-[#8E8E93]">尚未选择图纸</div>}</div></section>
...
function Modal({ title, children, onClose, wide }: { title: string; children: ReactNode; onClose: () => void; wide?: boolean }) { return <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/35 p-4" onMouseDown={onClose}><div className={`max-h-[90vh] overflow-y-auto bg-white p-5 shadow-2xl ${wide ? "w-full max-w-4xl" : "w-full max-w-lg"}`} onMouseDown={(event) => event.stopPropagation()}><div className="flex items-center justify-between gap-4"><h3 className="font-semibold">{title}</h3><button onClick={onClose} aria-label="关闭" className="h-8 w-8 text-xl text-[#8E8E93] hover:bg-[#F2F2F7]">×</button></div><div className="mt-4">{children}</div></div></div>; }
