...
export default function ProductionPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      setLoading(true);
      try { await loadAll(); }
      catch (error) { if (!cancelled) notify(apiMessage(error, "履约数据加载失败"), true); }
...
function TechnicalEditor({ door, busy, notify, onBusy, onChanged }: WorkbenchProps) {
  ...
  useEffect(() => {
    let active = true;
    getInventoryMaterials().then((items) => active && setMaterials(items)).catch((error) => notify(apiMessage(error, "物料档案加载失败"), true));
    return () => { active = false; };
  }, [notify]);
  ...
  const addComponent = () => setComponents([...components, { material_id: null, name: "", category: "其他", specification: "", quantity: 1, unit: "件", acquisition_method: "待确定", remark: "" }]);
  ...
  return <div className="space-y-5">
    <div className="grid gap-3 md:grid-cols-2">
      <label className="text-xs text-[#636366]">产品摘要<textarea disabled={!editable} value={summary} onChange={(event) => setSummary(event.target.value)} className="mt-1 min-h-20 w-full border border-[#C7C7CC] p-2 text-sm text-[#1C1C1E] disabled:bg-[#F2F2F7]" /></label>
      <label className="text-xs text-[#636366]">特殊要求<textarea disabled={!editable} value={requirements} onChange={(event) => setRequirements(event.target.value)} className="mt-1 min-h-20 w-full border border-[#C7C7CC] p-2 text-sm text-[#1C1C1E] disabled:bg-[#F2F2F7]" /></label>
    </div>
    <EditableTable title="构件清单" editable={editable} onAdd={addComponent} columns={["关联物料", "构件名称", "分类", "规格", "数量", "单位", "取得方式", "操作"]}>
      {components.map((item, index) => <tr key={item.id || index} className="border-t border-[#E5E5EA]">
        <MaterialCell disabled={!editable} value={item.material_id} materials={materials} onChange={(material) => updateAt(components, setComponents, index, { material_id: material?.id || null, material_code: material?.code || null, material_name: material?.name || null, specification: material?.specification || item.specification, unit: material?.unit || item.unit, name: item.name || material?.name || "" })} />
        <CellInput disabled={!editable} value={item.name} onChange={(value) => updateAt(components, setComponents, index, { name: value })} />
        <CellInput disabled={!editable} value={item.category} onChange={(value) => updateAt(components, setComponents, index, { category: value })} />
        <CellInput disabled={!editable} value={item.specification} onChange={(value) => updateAt(components, setComponents, index, { specification: value })} />
        <CellInput disabled={!editable} type="number" value={item.quantity} onChange={(value) => updateAt(components, setComponents, index, { quantity: Number(value) || 0 })} />
        <CellInput disabled={!editable} value={item.unit} onChange={(value) => updateAt(components, setComponents, index, { unit: value })} />
        <CellInput disabled={!editable} value={item.acquisition_method} onChange={(value) => updateAt(components, setComponents, index, { acquisition_method: value })} />
        <td className="px-2 py-2">{editable && <button onClick={() => setComponents(components.filter((_, i) => i !== index))} className="text-xs text-[#C62828]">删除</button>}</td>
      </tr>)}
    </EditableTable>
    <EditableTable title="工作包" editable={editable} onAdd={addWork} columns={["名称", "类别", "路线", "取得方式", "执行人", "计划完成", "操作"]}>
      {works.map((item,index) => <tr key={item.id || index} className="border-t border-[#E5E5EA]"><CellInput disabled={!editable} value={item.name} onChange={(value) => updateAt(works,setWorks,index,{name:value})} /><CellInput disabled={!editable} value={item.category} onChange={(value) => updateAt(works,setWorks,index,{category:value})} /><CellInput disabled={!editable} value={item.route} onChange={(value) => updateAt(works,setWorks,index,{route:value})} /><CellInput disabled={!editable} value={item.acquisition_method} onChange={(value) => updateAt(works,setWorks,index,{acquisition_method:value})} /><CellInput disabled={!editable} value={item.executor_uid} onChange={(value) => updateAt(works,setWorks,index,{executor_uid:value})} /><CellInput disabled={!editable} type="date" value={item.planned_end} onChange={(value) => updateAt(works,setWorks,index,{planned_end:value})} /><td className="px-2 py-2">{editable && <button onClick={() => setWorks(works.filter((_,i)=>i!==index))} className="text-xs text-[#C62828]">删除</button>}</td></tr>)}
    </EditableTable>
    {editable ? <div className="flex justify-end gap-2"><button disabled={busy} onClick={() => void persist()} className="h-9 border border-[#C7C7CC] px-4 text-sm">保存草稿</button><button disabled={busy} onClick={() => void persist(true)} className="h-9 bg-[#007AFF] px-4 text-sm text-white">确认并冻结 V{source.version}</button></div> : <div className="flex items-center justify-between border border-[#B7E0BF] bg-[#F2FFF3] p-3 text-sm"><span>技术包 V{source.version} 已冻结，工作包和物料需求已释放。</span><ChangeButton door={door} notify={notify} onBusy={onBusy} onChanged={onChanged} /></div>}
  </div>;
}
...
function WorkBoard({ door, busy, notify, onBusy, onChanged }: WorkbenchProps) {
  ...
  const runBatch = async (action: string) => {
    if (!selectedActive.length) return notify("请先勾选工作包", true);
    ...
    onBusy(true);
    try { const result = await batchFulfillmentWorkPackages(door.id, { work_ids: selectedActive, action, executor_uid: executor, remark: actionRemark }); notify(result.message); setSelected([]); setRemark(""); await onChanged(result.door_unit); }
    catch (error) { notify(apiMessage(error, "批量更新工作包失败"), true); }
    finally { onBusy(false); }
  };
  return <div className="space-y-3"><section className="sticky top-14 z-20 border border-[#B8D8F8] bg-[#F4F9FF] p-3"><div className="flex flex-wrap items-center gap-2"><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={activeIds.length > 0 && selectedActive.length === activeIds.length} onChange={() => setSelected(selectedActive.length === activeIds.length ? [] : activeIds)} />全选未完成</label><span className="text-xs text-[#636366]">已选 {selectedActive.length} 项</span><input list="fulfillment-people" value={executor} onChange={(event) => setExecutor(event.target.value)} placeholder="统一执行人（可空）" className="h-9 w-44 border border-[#C7C7CC] bg-white px-2 text-sm"/><input value={remark} onChange={(event) => setRemark(event.target.value)} placeholder="批量说明（跳过时必填）" className="h-9 min-w-52 flex-1 border border-[#C7C7CC] bg-white px-2 text-sm"/>{["开始","提交质检","确认完成","跳过"].map((action)=><button key={action} disabled={busy||!selectedActive.length} onClick={()=>void runBatch(action)} className={`h-9 px-3 text-sm text-white disabled:opacity-40 ${action === "跳过" ? "bg-[#8E8E93]" : action === "确认完成" ? "bg-[#248A3D]" : "bg-[#007AFF]"}`}>{action}</button>)}</div><p className="mt-2 text-xs text-[#636366]">无需逐级点击：不要求过程检验的工作包可直接确认完成；要求检验的工作包先提交质检。跳过必须留原因。</p></section><div className="grid gap-3 lg:grid-cols-3">{["待排单","已排单","进行中","待质检","已完成","暂停","异常","返工"].map((status) => <section key={status} className="border border-[#D1D1D6] bg-[#F8F8FA]"><div className="flex items-center justify-between border-b border-[#D1D1D6] px-3 py-2 text-sm font-semibold"><span>{status}</span><span className="text-xs text-[#636366]">{grouped[status]?.length || 0}</span></div><div className="space-y-2 p-2">{(grouped[status] || []).map((work) => <WorkCard key={work.id} work={work} busy={busy} selected={Boolean(work.id && selectedActive.includes(work.id))} onSelect={(checked)=>work.id&&setSelected(checked?[...selectedActive,work.id]:selectedActive.filter((id)=>id!==work.id))} onSave={async (payload) => { if (!work.id) return; onBusy(true); try { const result=await updateFulfillmentWorkPackage(work.id,payload); notify(result.message); await onChanged(result.door_unit); } catch(error){ notify(apiMessage(error,"工作包更新失败"),true); } finally{onBusy(false);} }} />)}{!grouped[status]?.length && <div className="py-5 text-center text-xs text-[#8E8E93]">暂无</div>}</div></section>)}</div></div>;
}
...
function DeliveryBoard({ door, busy, notify, onBusy, onChanged }: WorkbenchProps) {
  const [pay, setPay] = useState({ amount: 0, payment_date: new Date().toISOString().slice(0, 10), reference: "", remark: "" });
  const [ship, setShip] = useState({ required_payment: 0, carrier: "", vehicle_no: "", contact: "", authorization_reason: "", authorized_by: "", remark: "" });
  const finance = door.sales_finance;
  const submitPayment = async () => { onBusy(true); try { const result = await recordFulfillmentPayment(door.id, pay); notify(result.message); setPay({ ...pay, amount: 0, reference: "" }); await onChanged(result.door_unit); } catch (error) { notify(apiMessage(error, "收款登记失败"), true); } finally { onBusy(false); } };
  const submitShipment = async () => { onBusy(true); try { const result = await createFulfillmentShipment(door.id, ship); notify(result.message); await onChanged(result.door_unit); } catch (error) { notify(apiMessage(error, "发货失败"), true); } finally { onBusy(false); } };
  const sign = async (shipmentId: number) => { const signedBy = window.prompt("请输入签收人") || "客户"; onBusy(true); try { const result = await signFulfillmentShipment(shipmentId, { signed_by: signedBy, signed_at: "", remark: "" }); notify(result.message); await onChanged(result.door_unit); } catch (error) { notify(apiMessage(error, "签收登记失败"), true); } finally { onBusy(false); } };
  return <div className="space-y-4"><div className="grid gap-4 lg:grid-cols-2">
    {finance ? <section className="border border-[#D1D1D6] p-4"><div className="flex items-center justify-between"><h3 className="text-sm font-semibold">订单实收</h3><strong className={finance.unpaid_amount > 0 ? "text-[#C62828]" : "text-[#248A3D]"}>{finance.unpaid_amount > 0 ? `未收 ¥${finance.unpaid_amount.toFixed(2)}` : "已收齐"}</strong></div><div className="mt-3 grid grid-cols-3 gap-2 bg-[#F8F8FA] p-3 text-xs"><span>订单<br/><strong>{finance.order_no}</strong></span><span>总额<br/><strong>¥{finance.total_amount.toFixed(2)}</strong></span><span>已收<br/><strong className="text-[#248A3D]">¥{finance.paid_amount.toFixed(2)}</strong></span></div><p className="mt-3 text-xs text-[#636366]">实际收款统一在“订单确认”登记，可将一笔收款分配到多张 TM 订单。</p></section> : <section className="border border-[#D1D1D6] p-4"><div className="flex items-center justify-between"><h3 className="text-sm font-semibold">收款登记</h3><strong className="text-[#248A3D]">已收 ¥{door.paid_amount.toFixed(2)}</strong></div><div className="mt-3 grid grid-cols-2 gap-2"><input type="number" value={pay.amount} onChange={(event) => setPay({ ...pay, amount: Number(event.target.value) || 0 })} placeholder="收款金额" className="h-9 border border-[#C7C7CC] px-2 text-sm"/><input type="date" value={pay.payment_date} onChange={(event) => setPay({ ...pay, payment_date: event.target.value })} className="h-9 border border-[#C7C7CC] px-2 text-sm"/><input value={pay.reference} onChange={(event) => setPay({ ...pay, reference: event.target.value })} placeholder="凭证号" className="h-9 border border-[#C7C7CC] px-2 text-sm"/><button disabled={busy || pay.amount <= 0} onClick={() => void submitPayment()} className="h-9 bg-[#007AFF] text-sm text-white disabled:bg-[#C7C7CC]">登记收款</button></div></section>}
    <section className="border border-[#D1D1D6] p-4"><h3 className="text-sm font-semibold">发货与财务放行</h3><div className="mt-3 grid grid-cols-2 gap-2">{!finance && <input type="number" value={ship.required_payment} onChange={(event) => setShip({ ...ship, required_payment: Number(event.target.value) || 0 })} placeholder="本次发货应收" className="h-9 border border-[#C7C7CC] px-2 text-sm"/>}<input value={ship.carrier} onChange={(event) => setShip({ ...ship, carrier: event.target.value })} placeholder="物流/司机" className="h-9 border border-[#C7C7CC] px-2 text-sm"/><input value={ship.vehicle_no} onChange={(event) => setShip({ ...ship, vehicle_no: event.target.value })} placeholder="车牌/运单" className="h-9 border border-[#C7C7CC] px-2 text-sm"/><input value={ship.contact} onChange={(event) => setShip({ ...ship, contact: event.target.value })} placeholder="联系电话" className="h-9 border border-[#C7C7CC] px-2 text-sm"/><input value={ship.authorization_reason} onChange={(event) => setShip({ ...ship, authorization_reason: event.target.value })} placeholder="欠款放行原因（可空）" className="h-9 border border-[#C7C7CC] px-2 text-sm"/><input value={ship.authorized_by} onChange={(event) => setShip({ ...ship, authorized_by: event.target.value })} placeholder="授权人（放行时必填）" className="h-9 border border-[#C7C7CC] px-2 text-sm"/><button disabled={busy} onClick={() => void submitShipment()} className="col-span-2 h-9 bg-[#007AFF] text-sm text-white">确认出库发货</button></div></section>
  </div><SimpleRecords title="发货记录" rows={door.shipments.map((item) => [item.status, item.carrier || "自送", item.vehicle_no || "-", item.authorized ? `授权：${item.authorized_by}` : `收款 ¥${item.paid_amount}`, formatTime(item.created_at)])}/>{door.shipments.filter((item) => item.status !== "已签收").map((item) => <div key={item.id} className="flex items-center justify-between border border-[#B8D8F8] bg-[#F4F9FF] p-3 text-sm"><span>{item.carrier || "自送"} · {item.vehicle_no || "无车牌/运单"} · 运输中</span><button disabled={busy} onClick={() => void sign(item.id)} className="h-8 bg-[#248A3D] px-3 text-xs text-white">登记签收</button></div>)}<SimpleRecords title="计件工资草稿" rows={door.payroll_drafts.map((item) => [item.employee_uid || "未指定", item.work_name, `${item.quantity} × ¥${item.piece_rate}`, `¥${item.amount.toFixed(2)}`, item.status])}/></div>;
}
...
function MaterialCell({disabled,value,materials,onChange}:{disabled:boolean;value?:number|null;materials:InventoryMaterial[];onChange:(material:InventoryMaterial|null)=>void}) { return <td className="px-2 py-2"><select disabled={disabled} value={value || ""} onChange={(event)=>onChange(materials.find((item)=>item.id===Number(event.target.value))||null)} className="h-8 w-full border border-[#C7C7CC] px-1 text-xs disabled:border-transparent disabled:bg-transparent"><option value="">未关联</option>{materials.map((material)=><option key={material.id} value={material.id}>{material.code} · {material.name}{material.specification?` · ${material.specification}`:""}</option>)}</select></td>; }
