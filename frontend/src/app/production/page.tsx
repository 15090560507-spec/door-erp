...
const WORK_STATUS_OPTIONS = ["待排单", "已排单", "进行中", "待质检", "已完成", "暂停", "异常", "返工", "已取消"];
...
function WorkBoard({ door, busy, notify, onBusy, onChanged }: WorkbenchProps) {
  ...
  return <div className="grid gap-3 lg:grid-cols-3">{["待排单","已排单","进行中","待质检","已完成","暂停","异常","返工"].map((status) => <section key={status} className="border border-[#D1D1D6] bg-[#F8F8FA]"><div className="flex items-center justify-between border-b border-[#D1D1D6] px-3 py-2 text-sm font-semibold"><span>{status}</span><span className="text-xs text-[#636366]">{grouped[status]?.length || 0}</span></div><div className="space-y-2 p-2">{(grouped[status] || []).map((work) => <WorkCard key={work.id} work={work} busy={busy} onSave={async (payload) => { if (!work.id) return; onBusy(true); try { const result=await updateFulfillmentWorkPackage(work.id,payload); notify(result.message); await onChanged(result.door_unit); } catch(error){ notify(apiMessage(error,"工作包更新失败"),true); } finally{onBusy(false);} }} />)}{!grouped[status]?.length && <div className="py-5 text-center text-xs text-[#8E8E93]">暂无</div>}</div></section>)}</div>;
}

function WorkCard({ work, busy, onSave }: { work: FulfillmentWorkPackage; busy: boolean; onSave: (payload: {status:string;executor_uid:string;actual_quantity?:number;remark:string})=>Promise<void> }) { const [next,setNext]=useState(work.status||"待排单"); const [executor,setExecutor]=useState(work.executor_uid||""); const [remark,setRemark]=useState(work.remark||""); return <div className="border border-[#D1D1D6] bg-white p-3"><div className="text-sm font-semibold">{work.name}</div><div className="mt-1 text-xs text-[#636366]">{work.route||work.category} · {work.acquisition_method}</div><div className="mt-3 grid gap-2"><input value={executor} onChange={(e)=>setExecutor(e.target.value)} placeholder="执行人" className="h-8 border border-[#C7C7CC] px-2 text-xs"/><select value={next} onChange={(e)=>setNext(e.target.value)} className="h-8 border border-[#C7C7CC] px-2 text-xs">{WORK_STATUS_OPTIONS.map(item=><option key={item}>{item}</option>)}</select><input value={remark} onChange={(e)=>setRemark(e.target.value)} placeholder="完成说明/异常说明" className="h-8 border border-[#C7C7CC] px-2 text-xs"/><button disabled={busy||next===work.status} onClick={()=>void onSave({status:next,executor_uid:executor,actual_quantity:next==="已完成"?work.quantity:undefined,remark})} className="h-8 bg-[#007AFF] text-xs text-white disabled:bg-[#C7C7CC]">提交状态</button></div></div>; }
